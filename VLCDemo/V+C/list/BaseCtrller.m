...
- (void)prepare {
    ...
    while ((pathTmp = [dirEnum nextObject]) != nil) {
        ...
        if ([pathTmp containsString:@".DS_Store"])
            continue;
        else if ([pathTmp containsString:@".sqlite"])
            continue;
        else if ([pathTmp containsString:@"cover"])
            continue;
        else if ([self isPhotoType:pathTmp])
            continue;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.fType == typeOfFileModel_folder) { // is folder
